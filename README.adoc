...
=== Contributing New Anchors via Pull Request

Once you've tested a semantic anchor and confirmed it's valuable, you can contribute it to your catalog using a Git workflow.

==== Step 1: Prepare Your Branch

Start from the main branch and create a feature branch for your addition:
...
git checkout -b feature/add-semantic-anchor-xyz
----

IMPORTANT: Always start feature branches from `main`, not from other feature branches.

==== Step 2: Add Your Semantic Anchor

Edit the `semantic-anchors-for-llms.adoc` file and add a new section following the established pattern:
...
----
# Stage your changes
git add semantic-anchors-for-llms.adoc

# Commit with a descriptive message
git commit -m "Add semantic anchor: Your Anchor Name"

# Consider adding co-authorship if Claude helped
git commit --amend --no-edit --trailer "Co-authored-by: Claude <claude@anthropic.com>"
----

NOTE: Decide whether to credit Claude as co-author based on the level of contribution.
...
----
# Push your branch
git push origin feature/add-semantic-anchor-xyz

# Create a PR via GitHub/GitLab UI or CLI
gh pr create --title "Add semantic anchor: Your Anchor Name" \
             --body "Adds semantic anchor for XYZ methodology. 
             
Tested with LLM and confirmed good recognition."
----

==== Step 5: Review and Merge

* Have the PR reviewed by team members
* Ensure the anchor fits the catalog structure
* Check for duplicate or overlapping anchors
* Merge to main once approved

==== Working with Forks

If you're contributing to a forked repository:

1. *Check fork status*: Ensure your fork is synchronized with upstream
+
[source,bash]
----
...
2. *Create branch*: Always branch from the updated main
3. *Create PR*: Submit PR to the upstream repository
