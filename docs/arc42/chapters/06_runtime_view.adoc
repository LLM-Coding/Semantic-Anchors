...
App -> App: Rendert Treemap
App --> User: Zeigt Homepage mit Treemap
@enduml
----
...
participant "Treemap Component" as Treemap
participant "LocalStorage" as LS

User -> Filter: Wählt "Software Developer"
Filter -> State: updateFilters({roles: ['software-developer']})
State -> State: Filtert Anchors
State -> Treemap: update(filteredAnchors)
Treemap -> Treemap: Generiert neue Treemap-Daten
Treemap --> User: Zeigt gefilterte Treemap
...
@startuml
actor User
participant "Treemap/Search" as UI
participant Router
...
Chart --> User: Treemap mit neuem Theme
